...
| Refactor | Invert Sandbox: `SysProcAttr()` provider → `Command(Spec)` constructor | [ ] todo |
| F-B | `nsjailSandbox`: seccomp, tmpfs-capped `/tmp`, `no_new_privs`, per-jail nproc | [ ] todo |
...
- [ ] Define `Spec` (argv, workdir, timeouts, rlimits, per-run nproc, fsize).
- [ ] Replace `Sandbox.SysProcAttr()` with `Sandbox.Command(Spec) *exec.Cmd`.
- [ ] Port `netnsSandbox` to the new interface (behaviour-preserving).
- [ ] Confirm `executor` compiles unchanged against the new seam.

## F-B — nsjail sandbox

Fixes **R2** (no seccomp), **R3** (`/tmp` OOM), **R4** (no `no_new_privs`).

- [ ] `nsjailSandbox` implementing `Command(Spec)`.
- [ ] Args: `--tmpfsmount /tmp:size=<n>m`, `--rlimit_nproc`, `--rlimit_fsize`,
      `--rlimit_as/cpu`, `--time_limit`, `--seccomp_string`, `no_new_privs` on.
- [ ] Dockerfile: build nsjail from source into the runtime image.
- [ ] Config flag to select `netns` vs `nsjail` backend (fail-closed default).
- [ ] Netns + execution test suites pass against the nsjail backend.
...
| R2 | No seccomp filter | F-B |
| R3 | `/tmp` unbounded → host OOM | F-B |
| R4 | No `no_new_privs` | F-B |
